...
// The external YAML editor subsystem (previously ~960 lines inline here)
// lives in ./modules/externalYamlEditor.js. We hand it the four kind
// descriptors plus the shared helpers it needs from this page (busy
// spinner, config-name lookup, modal preparer), and it hands back
// updateExternalYamlEditButton for the row builders to call.
const externalYamlEditor = initExternalYamlEditorSubsystem({
  kinds: {
    metadata_files: {
      label: 'Metadata file',
      ...
      setStatus: (row, kind, message) => setMetadataFileStatus(row, kind, message),
      sync: (editor, emitEvents) => syncMetadataFilesEditor(editor, emitEvents)
    },
    collection_files: {
      ...
      setStatus: (row, kind, message) => setCollectionFileStatus(row, kind, message),
      sync: (editor, emitEvents) => syncCollectionFilesEditor(editor, emitEvents)
    },
    overlay_files: {
      label: 'Overlay file',
      ...
      setStatus: (row, kind, message) => setOverlayFileStatus(row, kind, message),
      sync: (editor, emitEvents) => syncOverlayFilesEditor(editor, emitEvents)
    },
    playlist_files: {
      label: 'Playlist file',
      ...
      setStatus: (row, kind, message) => setPlaylistFileStatus(row, kind, message),
      sync: (editor, emitEvents) => syncPlaylistFilesEditor(editor, emitEvents)
    }
  },
  getActiveConfigName: () => getActiveConfigName(),
  setLibrariesButtonPersistentBusy: (button, busy, label) => setLibrariesButtonPersistentBusy(button, busy, label),
  prepareLibrariesModal: (modalEl) => prepareLibrariesModal(modalEl)
})
const updateExternalYamlEditButton = externalYamlEditor.updateExternalYamlEditButton
