...
function getSelectedOverlaysByType () {
  const overlayData = { show: [], season: [], episode: [] }

  document.querySelectorAll('.overlay-group').forEach(group => {
    const type = group.dataset.type
    group.querySelectorAll('.overlay-toggle:checked').forEach(toggle => {
      overlayData[type].push(toggle.value)
    })
  })
...
function generatePreview ({ libraryId, type, selectedImage, overlays }) {
  if (!libraryId || !type || !selectedImage) {
  ...
  const payload = {
    ...
    overlays: Array.isArray(overlays) ? overlays : (overlays[type] || [])
  }
  ...
  fetch('/generate_preview', {
    ...
    .then(data => {
      if (data.status === 'success') {
        const img = document.querySelector(`#${type}-preview`)
        if (img) {
          img.src = `${data.preview_url}?${Date.now()}`
        } else {
          console.warn('Preview image element not found for:', type)
        }
      } else {
        console.error('Preview generation failed:', data.message)
      }
    })
    .catch(err => {
...
document.addEventListener('DOMContentLoaded', function () {
  ['movie', 'show', 'season', 'episode'].forEach(type => {
    const selector = document.querySelector(`#${type}-image-selector`)
    const libraryId = document.querySelector('#library-id')?.value

    const handlePreviewUpdate = () => {
      generatePreview({
        type,
        selectedImage: selector?.value || '',
        overlays: getSelectedOverlaysByType(),
        libraryId
      })
    }

    document.querySelectorAll(`.overlay-group[data-type="${type}"] .overlay-toggle`).forEach(toggle => {
      toggle.addEventListener('change', handlePreviewUpdate)
    })

    if (selector) {
      selector.addEventListener('change', handlePreviewUpdate)
    }
  })
})
