...
def deep_merge_dicts(old, new):
    """
    Recursively merges two dictionaries. Values from `new` overwrite `old`,
    except when keys are missing in `new` — in which case `old` is preserved.
    """
    ...
    for key, new_value in new.items():
        ...
        if isinstance(old_value, dict) and isinstance(new_value, dict):
            merged[key] = deep_merge_dicts(old_value, new_value)
        else:
            merged[key] = new_value
    return merged


def parse_nested_form_data(flat_form):
    """
    Reconstructs nested dictionaries from flat form keys with square brackets.
    Example:
        flat_form = {
            'libraries[mov-library_movies][library]': 'Movies',
            'libraries[mov-library_movies][overlays][ribbon]': 'true'
        }
    Returns:
        {
            'libraries': {
    ...
    for flat_key, value in flat_form.items():
        # Extract parts of keys like "libraries[mov-library_movies][overlays][ribbon]"
        parts = re.findall(r'[^\[\]]+', flat_key)
        ref = nested_data
